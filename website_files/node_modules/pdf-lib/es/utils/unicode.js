...
export var utf8Encode = function utf8Encode(input, byteOrderMark) {
...
export var utf16Encode = function utf16Encode(input, byteOrderMark) {
    if (byteOrderMark === void 0) {
        byteOrderMark = true;
    }
...
export var isWithinBMP = function isWithinBMP(codePoint) {
...
export var hasSurrogates = function hasSurrogates(codePoint) {
    return codePoint >= 0x010000 && codePoint <= 0x10ffff;
}; // From Unicode 3.0 spec, section 3.7:
//   http://unicode.org/versions/Unicode3.0.0/ch03.pdf

export var highSurrogate = function highSurrogate(codePoint) {
    return Math.floor((codePoint - 0x10000) / 0x400) + 0xd800;
}; // From Unicode 3.0 spec, section 3.7:
//   http://unicode.org/versions/Unicode3.0.0/ch03.pdf

export var lowSurrogate = function lowSurrogate(codePoint) {
...
export var utf16Decode = function utf16Decode(input, byteOrderMark) {
    if (byteOrderMark === void 0) {
        byteOrderMark = true;
...
var isHighSurrogate = function isHighSurrogate(codePoint) {
...
var isLowSurrogate = function isLowSurrogate(codePoint) {
...
var decodeValues = function decodeValues(first, second, byteOrder) {
...
var readBOM = function readBOM(bytes) {
...
var hasUtf16BigEndianBOM = function hasUtf16BigEndianBOM(bytes) {
    return bytes[0] === 0xfe && bytes[1] === 0xff;
};

var hasUtf16LittleEndianBOM = function hasUtf16LittleEndianBOM(bytes) {
    return bytes[0] === 0xff && bytes[1] === 0xfe;
};

export var hasUtf16BOM = function hasUtf16BOM(bytes) {
